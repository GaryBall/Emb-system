var game = new Phaser.Game(800,500,Phaser.AUTO,'game'); //实例化game
...
game.States.boot = function(){
	this.preload = function(){
		...
		game.load.image('loading','assets/preloader.gif');//加载正在加载的gif
	};
...
game.States.preload = function(){
	this.preload = function(){
		...
		//以下为要加载的资源
        game.load.image('bg_combined','assets/bg_combined.png'); //背景
		game.load.image('background','assets/background.png'); //背景
    	game.load.image('ground','assets/ground1.png'); //地面
    	game.load.image('title','assets/title.png'); //游戏标题
    	game.load.spritesheet('bird','assets/pappu.png',60,60,8); //鸟
    	game.load.image('btn','assets/start-button.png');  //按钮
        game.load.image('pakia','assets/angry_pakia.png');  //pakia
        game.load.image('happy_pakia','assets/happy_pakia.png');  //pakia
...
game.States.play = function(){
	this.create = function(){
		this.bg = game.add.tileSprite(0,0,game.width,game.height,'bg_combined');//背景图
		this.pipeGroup = game.add.group();
		this.angry_pkGroup = game.add.group();
		this.angry_pkGroup.enableBody = true;
        this.happy_pkGroup = game.add.group();
        this.happy_pkGroup.enableBody = true;
		...
		this.timer1 = game.time.events.loop(1200, this.generatePipes, this);
	...
	this.update = function(){
        ...
        game.physics.arcade.overlap(this.bird, this.angry_pkGroup, this.hit_angry_pk, null, this); //与Pakia碰撞
        game.physics.arcade.overlap(this.bird, this.happy_pkGroup, this.hit_happy_pk, null, this); //与Pakia碰撞
    ...
    this.hit_angry_pk = function(){
        if(this.gameIsOver) return;
        immortal = true;
        game.time.events.add(Phaser.Timer.SECOND*2,this.nothing,this);
    }
    ...
    this.hit_happy_pk = function(){
        if(this.gameIsOver) return;
        pipe_coe = 0.5
        game.time.events.add(Phaser.Timer.SECOND*2,this.slow,this);
        //this.pipeGroup.setAll('body.velocity.x', -this.gameSpeed*0.5);
	...
	this.generatePipes = function(gap){ //制造管道
		...
		//通过随机数来控制生成怪物
		if(random_num<0.5) {
		    var angry_pakia = game.add.sprite(game.width-50, (random_num-0.5)*100 + 200, 'pakia', 0, this.angry_pkGroup);
		    }
        else{
        	var random2 = Math.random()
        	if(random2 < 0.2) {
            var happy_pakia = game.add.sprite(game.width-50, (random2-0.5)*100 + 200, 'happy_pakia', 0, this.happy_pkGroup);
        	}
        }
		this.pipeGroup.setAll('checkWorldBounds',true);
		this.pipeGroup.setAll('outOfBoundsKill',true);
		this.pipeGroup.setAll('body.velocity.x', -this.gameSpeed*pipe_coe);
        this.angry_pkGroup.setAll('body.velocity.x', -this.gameSpeed*1.2);
        this.angry_pkGroup.setAll('body.velocity.y',(random_num-0.5)*100);
        this.angry_pkGroup.setAll('outOfBoundsKill',true);
        this.happy_pkGroup.setAll('body.velocity.x', -this.gameSpeed*1.1);
        this.happy_pkGroup.setAll('body.velocity.y',(random2-0.5)*100);
        this.happy_pkGroup.setAll('outOfBoundsKill',true);
